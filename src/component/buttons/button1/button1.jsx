...
const Button1 = ({ value, handleClick, disable, adClass, data, classCustom }) => {
  ...
  return (
    <>
      <button
        className={`m-0 btn userBtn  ${classCustom ? classCustom : ''} text-decoration-none ${data == 0 && 'w-100' } ${value?.prices?.length > 0 && 'w-100'} ${adClass ? 'active' : ''}`}
        onClick={handleClick}
        disabled={disable}
      >
